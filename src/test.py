...
import subprocess
import shutil
from src.type import CommandArg, State, ChangedRule, UtilType
...
def get_test_command_arg(*args) -> CommandArg:
...
def get_changed_rules(arg: CommandArg) -> List[ChangedRule]:
...
def init_tmp_file(arg: CommandArg, changedRules: List[ChangedRule]):
    # copy before-rules
...
def init_docker_compose_file(arg: CommandArg, state: State):
...
def runner(args: CommandArg, changedRules: List[ChangedRule], state: State):
